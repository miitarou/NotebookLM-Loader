...
### 4. Smart Chunking (統合・分割機能) (New)
- **目的**: NotebookLMの1ファイルあたりの容量制限（目安）や、ファイル数過多を回避する。
- **仕様**:
    - **出力先**: `converted_files_merged/`
    - **動作**: 変換されたMarkdownテキストを結合していく。
    - **分割ルール**: 
        - 蓄積バッファが **20万文字** を超えたら `Merged_Files_Vol1.md` を書き出し、次は `Vol2` へ。
        - **巨大ファイル対応**: もし1ファイルだけで20万文字を超える場合、そのファイル自体を `(Part 1/n)` として分割し、強制的にチャンクに収める。
    - **目次**: 各Volの冒頭に、収録ファイル一覧を記載。
